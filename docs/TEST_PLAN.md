...
| # | 점검항목 | 방법 | 예상결과 | 결과 |
|---|---------|------|---------|------|
| 6-1 | 공유 UI 표시 | 일정 생성 모달에서 "일정 공유" 드롭다운 확인 | 같은 본부 소속 처/실 목록 표시 | |
| 6-2 | 처/실 선택 | 드롭다운에서 체크박스로 다중 선택 | 선택된 처/실 태그 표시, 개수 메시지 갱신 | |
| 6-3 | 공유 일정 생성 | 2개 이상 처/실 선택 후 일정 저장 | 생성 성공 | |
| 6-4 | 공유 확인 | 상세 모달에서 공유된 처/실 목록 확인 | Users 아이콘과 공유 처/실 이름 표시 | |
| 6-5 | 타 처/실 조회 | 다른 처/실 소속 계정으로 로그인 → 공유 일정 확인 | 공유된 일정이 캘린더에 표시 | |
...
| # | 점검항목 | 방법 | 예상결과 | 결과 |
|---|---------|------|---------|------|
| 8-1 | 전체 보기 | 하단 일정 목록 "전체" 탭 | 선택일/월 전체 일정 | |
| 8-2 | 진행중 필터 | "진행중" 탭 클릭 | PENDING 상태만 표시 | |
| 8-3 | 완료 필터 | "완료" 탭 클릭 | DONE 상태만 표시 | |
| 8-4 | 지연 필터 | "지연" 탭 클릭 | 마감 지난 PENDING 일정만 표시 (빨간색) | |
| 8-5 | 월간 보기 전환 | "전체 보기" 버튼 | 선택 날짜 해제, 이번 달 전체 일정 표시 | |
...
| 9B-6 | 소속원 있는 조직 삭제 | 소속 사용자 있는 조직 삭제 시도 | 삭제 거부 에러 메시지 | |
...
| # | 점검항목 | 방법 | 예상결과 | 결과 |
|---|---------|------|---------|------|
| 10-1 | USER 권한 | 일반 사용자 로그인 → 같은 부서 일정만 보이는지 확인 | 타 부서 일정 미표시, 관리자 아이콘 미표시 | |
| 10-2 | USER 수정 권한 | 타인 일정 상세 → 수정/삭제 버튼 확인 | 본인 일정만 수정 가능, 타인은 읽기 전용 | |
| 10-3 | ADMIN 전체 조회 | 관리자 로그인 → 모든 부서 일정 확인 | 모든 일정 조회 가능 | |
| 10-4 | ADMIN 타인 수정 | 관리자가 타인 일정 수정/삭제 | 가능 | |
| 10-5 | 비로그인 접근 | 로그아웃 상태에서 직접 URL 접근 | 로그인 페이지로 리다이렉트 | |
...
| # | 점검항목 | 방법 | 예상결과 | 결과 |
|---|---------|------|---------|------|
| 11-1 | 모바일 레이아웃 | 브라우저 폭 줄이기 또는 모바일 에뮬레이션 | 햄버거 메뉴, 캘린더 축소 표시 | |
| 11-2 | 다크모드 전체 | 다크모드에서 모든 페이지 순회 | 배경/텍스트/입력창 다크테마 일관성 | |
| 11-3 | 모달 닫기 | 모달 외부 클릭 또는 X 버튼 | 모달 정상 닫힘 | |
| 11-4 | 중복 클릭 방지 | 일정 생성 버튼 빠르게 연타 | 단일 요청만 발생 (중복 생성 방지) | |
...
| # | 점검항목 | 방법 | 예상결과 | 결과 |
|---|---------|------|---------|------|
| 12A-1 | PC 헤더에서 접근 | 헤더 우측 사용자 정보 영역 클릭 | 내 정보 수정 페이지로 이동 (hover 시 배경색 변경) | |
| 12A-2 | 모바일에서 접근 | 모바일 뷰에서 헤더 User 아이콘 클릭 | 내 정보 수정 페이지로 이동 | |
| 12A-3 | 돌아가기 | "돌아가기" 버튼 클릭 | 캘린더 페이지로 복귀 | |
| 12A-4 | 현재 정보 표시 | 프로필 페이지 상단 확인 | 이메일 (읽기전용), 역할 (읽기전용) 표시 | |
| 12A-5 | 폼 초기값 | 기본 정보 폼 확인 | 현재 이름/직급/본부/처/부서가 자동 채워져 있음 | |
...
| 12B-2 | 직급 변경 | 직급 드롭다운 변경 → 저장 | 성공 메시지, 헤더 직급 반영 | |
| 12B-3 | 소속 변경 | 본부→처→부서 순서대로 변경 → 저장 | 성공 메시지, 헤더 소속 정보 반영 | |
| 12B-4 | 조직 드롭다운 연동 | 본부 변경 시 처 목록 갱신, 처 변경 시 부서 목록 갱신 | 계층형 연동 정상 동작 | |
...
| # | 점검항목 | 방법 | 예상결과 | 결과 |
|---|---------|------|---------|------|
| 12C-1 | 비밀번호 변경 성공 | 현재 비밀번호 + 새 비밀번호 입력 → 변경 | 성공 메시지, 폼 초기화 | |
| 12C-2 | 변경 후 로그인 | 로그아웃 → 새 비밀번호로 로그인 | 로그인 성공 | |
| 12C-3 | 이전 비밀번호 거부 | 로그아웃 → 이전 비밀번호로 로그인 시도 | "이메일 또는 비밀번호가 올바르지 않습니다." | |
| 12C-4 | 현재 비밀번호 오류 | 잘못된 현재 비밀번호 입력 → 변경 시도 | "현재 비밀번호가 올바르지 않습니다." 에러 | |
| 12C-5 | 새 비밀번호 유효성 | 8자 미만 / 영문만 / 숫자만 입력 → 변경 시도 | 유효성 에러 메시지 | |
| 12C-6 | 비밀번호 확인 불일치 | 새 비밀번호 != 확인 상태에서 변경 시도 | "비밀번호가 일치하지 않습니다." 실시간 표시 | |
...
3. **3단계** → 캘린더 기본 동작
4. **4단계** → 단일 일정 전체 CRUD (4-8~4-10: 단일→반복 변환 포함)
5. **5단계** → 반복 일정 (핵심 기능)
6. **6단계** → 일정 공유
7. **7단계** → 알림
8. **2-3~2-11** → 회원가입 + 승인 워크플로우 (테스트 계정 생성 → 승인 대기 확인)
9. **9단계** → 관리자 기능 (9A-5~9A-9: 승인 워크플로우 점검)
10. **10단계** → 권한 테스트 (관리자 + 승인된 일반 사용자 2개 계정 활용)
11. **12단계** → 내 정보 수정 (프로필 변경 + 비밀번호 변경)
12. **8단계, 11단계** → 필터, UI

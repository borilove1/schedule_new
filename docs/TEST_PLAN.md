...
| 2 | 인증 기능 (로그인/회원가입/승인) | **PASS** | 회원가입→승인대기→로그인차단→관리자승인→로그인성공 워크플로우 정상, 부서 선택 필수화 |
...
| 6 | 일정 공유 | **PASS** | 드롭다운 20개 처/실 표시, 다중선택, 상세 공유표시, 타처 조회 정상, 직급 옵션 직할 부서 분리 |
...
| 12 | 내 정보 수정 (프로필) | **PASS** | 이름/직급/소속 변경, 비밀번호 변경/유효성검증 모두 정상, 부서 선택 필수화 |
...
| 2-4a | 부서 선택 필수화 | 기획관리실(부서 있는 처) 선택 후 직급 드롭다운 확인 | 부서 선택 전에는 직급 비활성화 + "부서를 먼저 선택하세요" 표시 | - |
| 2-4b | 부서 없는 처 직급 | 울산지사(부서 없는 처) 선택 후 직급 드롭다운 확인 | 바로 직급 선택 가능 (지사장, 부장, 차장, 직원) | - |
...
| 6-8 | 직급 옵션 (부서 전체) | 일정 공유에서 기획관리실 선택 → 부서 전체 → 직급 확인 | 부장, 차장, 직원만 표시 (실장 없음) | - |
| 6-9 | 직급 옵션 (직할 부서) | 일정 공유에서 기획관리실 선택 → 기획실 직할 → 직급 확인 | 실장만 표시 | - |
| 6-10 | 직급 옵션 (일반 부서) | 일정 공유에서 기획관리실 선택 → 전략경영부 → 직급 확인 | 부장, 차장, 직원만 표시 | - |
| 6-11 | 드롭다운 탭 포커스 | 처/부서/직급 드롭다운을 Tab 키로 이동 | 파란색 glow 효과 표시 | - |
...
| 12B-7 | 부서 선택 필수화 | 기획관리실(부서 있는 처) 선택 후 직급 드롭다운 확인 | 부서 선택 전에는 직급 비활성화 + "부서를 먼저 선택하세요" 표시 | - |
| 12B-8 | 직급 드롭다운 탭 포커스 | 본부/처/부서/직급 드롭다운을 Tab 키로 이동 | 파란색 glow 효과 표시 | - |

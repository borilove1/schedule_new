...
| 15 | 댓글 기능 | - | 백엔드 API 완성, 프론트엔드 UI 미구현 |
...
| 14A-3 | 일정 API Rate Limit | 1분 내 101회 이상 일정 API 호출 | 429 응답 | - |
| 14A-4 | Rate Limit 해제 후 | 제한 시간 경과 후 재시도 | 정상 응답 | - |

### 14-B. 프론트엔드 입력 검증

| # | 점검항목 | 방법 | 예상결과 | 결과 |
|---|---------|------|---------|------|
| 14B-1 | 일정 제목 필수 | 제목 없이 일정 생성 시도 | "제목을 입력해주세요" 프론트엔드 에러 | - |
| 14B-2 | 일정 시작일 필수 | 시작일 없이 일정 생성 시도 | "시작일을 선택해주세요" 에러 | - |
| 14B-3 | 시간 범위 검증 | 종료시간 < 시작시간 입력 | "종료 시간은 시작 시간 이후여야 합니다" 에러 | - |
| 14B-4 | 반복 종료일 검증 | 반복 종료일 < 시작일 입력 | "반복 종료일은 시작일 이후여야 합니다" 에러 | - |
| 14B-5 | 회원가입 이메일 형식 | 잘못된 이메일 형식 입력 | 프론트엔드 유효성 에러 | - |
| 14B-6 | 비밀번호 실시간 검증 | 비밀번호 입력 중 규칙 충족 여부 확인 | 영문+숫자 조합 8자 이상 실시간 피드백 | - |

---

## 15단계: 댓글 기능 (백엔드 API)

> **참고**: 프론트엔드 UI 미구현. API 레벨에서만 테스트 가능.

| # | 점검항목 | 방법 | 예상결과 | 결과 |
|---|---------|------|---------|------|
| 15-1 | 일정 댓글 작성 | `POST /api/v1/comments/events/:eventId` | 댓글 생성, 201 응답 | - |
| 15-2 | 시리즈 댓글 작성 | `POST /api/v1/comments/series/:seriesId` | 시리즈에 댓글 생성 | - |
| 15-3 | 댓글 수정 | `PUT /api/v1/comments/:id` (본인만) | 내용 변경, is_edited=true | - |
| 15-4 | 댓글 삭제 | `DELETE /api/v1/comments/:id` (본인 또는 ADMIN) | 댓글 삭제 | - |
| 15-5 | 타인 댓글 수정 시도 | 타인 댓글에 PUT 요청 | 403 권한 없음 | - |
| 15-6 | 비조회 일정 댓글 | 조회 권한 없는 일정에 댓글 시도 | canViewEvent 검증으로 거부 | - |
...
5. **4단계** → 단일 일정 전체 CRUD (4-8~4-10: 단일→반복 변환 포함)
6. **5단계** → 반복 일정 (핵심 기능)
7. **6단계** → 일정 공유
8. **7단계** → 알림
9. **2-3~2-11** → 회원가입 + 승인 워크플로우 (테스트 계정 생성 → 승인 대기 확인)
10. **9단계** → 관리자 기능 (9A-5~9A-9: 승인 워크플로우 점검)
11. **10단계** → 권한 테스트 (관리자 + 승인된 일반 사용자 2개 계정 활용)
12. **12단계** → 내 정보 수정 (프로필 변경 + 비밀번호 변경)
13. **8단계, 11단계** → 필터, UI
14. **14단계** → Rate Limiting & 입력 검증
15. **15단계** → 댓글 API 테스트
16. **16단계** → 부하 테스트 (선택)
